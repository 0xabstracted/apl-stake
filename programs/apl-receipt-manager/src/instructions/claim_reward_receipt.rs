...
use apl_payment_manager::{program::AplPaymentManager, state::PaymentManager};
use apl_stake_pool::state::StakeEntry;
...
pub struct CreateRewardReceiptCtx<'info> {
    ...
    apl_payment_manager: Program<'info, AplPaymentManager>,
...
pub fn handler(ctx: Context<CreateRewardReceiptCtx>) -> Result<()> {
    ...
    let cpi_accounts = apl_payment_manager::cpi::accounts::HandlePaymentCtx {
    ...
    let cpi_ctx = CpiContext::new(ctx.accounts.apl_payment_manager.to_account_info(), cpi_accounts);
    apl_payment_manager::cpi::manage_payment(cpi_ctx, *payment_amount)?;
